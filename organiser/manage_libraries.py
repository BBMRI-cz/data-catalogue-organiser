...
class LibrariesManager:
    ...
    def __init__(self, libraries_path, sample_sheet_path, path_to_run, predictive_number):
        ...
        self.parameters_path = os.path.join(path_to_run, "Samples", predictive_number, "Analysis", "Reports", f"{predictive_number}_Parameters.txt")
    ...
    def get_data_from_libraries(self):
        ...
        parameter = self._look_for_lib_in_parameters(possible_params)
        if parameter is None:
            samplesheet_df = pd.read_csv(self.sample_sheet_path)
            ...
            possible_names = [row["Panel"].lower() for _,row in df.iterrows() if experiment_name.lower() in row["Panel"] and self._date_in_date_range(row["Availability Date Range"], reference_date)]
            ...
            return self._create_dict_with_library_info(df, [], parameter=parameter)
    ...
    def _create_dict_with_library_info(self, dataframe, panel_value, look_for_manual=False, parameter=None):
        if parameter:
