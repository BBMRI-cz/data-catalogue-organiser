...
def test_run_is_in_correct_sturecture():
    ...
    assert os.path.exists(
        os.path.join(
            FAKE_DESTINATION_FILES, "2023", "NextSeq", "230101_N0000000_0000_0000000000"
        )
    )
        ...
        assert all(
            [
                ...
                for file in os.listdir(
                    os.path.join(sample_files, f"2023_000{i}_DNA", "FASTQ")
                )
            ]
        )
        assert (
            len(os.listdir(os.path.join(sample_files, f"2023_000{i}_DNA", "FASTQ")))
            == 8
        )
...
@pytest.mark.parametrize(
    "filename",
    ["SampleSheet.csv", "RunParameters.xml", "RunInfo.xml", "RunCompletionStatus.xml"],
)
...
def test_patient_correctly_created_in_tree():
    ...
    assert os.path.exists(
        os.path.join(
            FAKE_PATIENT_FILES,
            "2000",
            "mmci_patient_00000000-0000-0000-0000-000000000001",
