...
EXPECTED_ORGANISED_FILE_PATH = os.path.join(
...
def test_catalog_info_per_pred_number():
    ...
    assert os.path.exists(
        os.path.join(EXPECTED_ORGANISED_FILE_PATH, "catalog_info_per_pred_number")
    )
...
def test_analysis_files():
    ...
    for i in range(1, 20):
        ...
        expected_path_to_analysis = os.path.join(
            EXPECTED_ORGANISED_FILE_PATH, "Samples", predictive_path, "Analysis"
        )
        assert os.path.exists(expected_path_to_analysis)
        assert os.path.exists(os.path.join(expected_path_to_analysis, "Reports"))

        assert os.path.exists(
            os.path.join(expected_path_to_analysis, f"{predictive_path}.bam")
        )
        assert os.path.exists(
            os.path.join(expected_path_to_analysis, f"{predictive_path}.bam.bai")
        )

        assert os.path.exists(
            os.path.join(expected_path_to_analysis, f"{predictive_path}_Parameters.txt")
        )
        assert os.path.exists(
            os.path.join(expected_path_to_analysis, f"{predictive_path}_StatInfo.txt")
        )

        assert os.path.exists(
            os.path.join(
                expected_path_to_analysis,
                f"{predictive_path}_S6_L001_R1_001_convert.log",
            )
        )
        assert os.path.exists(
            os.path.join(
